...
# =============================================================================
# SMART QUEUE PROCESSING HELPER FUNCTIONS
# =============================================================================

async def smart_queue_processor():
    """Intelligent queue processor with health-based resource allocation"""
    try:
        current_time = datetime.now(timezone.utc)
        
        # Get pending requests ordered by priority and scheduled time
        pending_requests = await db.scraping_queue.find({
            "status": "pending",
            "scheduled_time": {"$lte": current_time}
        }).sort([("priority", 1), ("scheduled_time", 1)]).to_list(length=50)
        
        if not pending_requests:
            return {"processed": 0, "message": "No pending requests to process"}
        
        processed_count = 0
        for request in pending_requests:
            # Check if we have capacity
            current_processing = await db.scraping_queue.count_documents({"status": "processing"})
            if current_processing >= MAX_CONCURRENT_PROCESSING:
                break
            
            # Get healthy account and proxy
            account = await get_healthiest_available_account()
            proxy = await get_available_proxy()
            
            if not account:
                logger.warning("No healthy accounts available for processing")
                break
            
            # Assign resources and start processing
            await db.scraping_queue.update_one(
                {"id": request["id"]},
                {
                    "$set": {
        ...
        return {
            ...
            "message": f"Started processing {processed_count} requests"
        }
...
async def is_account_rate_limited(account_id: str) -> bool:
    ...
    try:
        ...
        current_time = datetime.now(timezone.utc)
        rate_limit_reset = account_doc.get("rate_limit_reset")
        
        if rate_limit_reset and rate_limit_reset > current_time:
            return True
        
        # Check hourly rate limits
        hourly_requests = await db.account_usage_logs.count_documents({
            "account_id": account_id,
            "timestamp": {"$gte": current_time - timedelta(hours=1)}
        })
        
        return hourly_requests >= MAX_REQUESTS_PER_HOUR_PER_ACCOUNT
        
    except Exception as e:
        logger.error(f"Error checking rate limit for account {account_id}: {e}")
        return True

async def detect_rate_limit_from_error(error_msg: str) -> bool:
    """Detect if error indicates rate limiting"""
    rate_limit_indicators = [
        "rate limit", "too many requests", "quota exceeded",
        "429", "throttled", "slow down"
    ]
    return any(indicator in error_msg.lower() for indicator in rate_limit_indicators)

async def detect_account_block_from_error(error_msg: str) -> bool:
    """Detect if error indicates account blocking"""
    block_indicators = [
        "account suspended", "account disabled", "banned",
        "access denied", "unauthorized", "403"
    ]
    return any(indicator in error_msg.lower() for indicator in block_indicators)

async def detect_ip_block_from_error(error_msg: str) -> bool:
    """Detect if error indicates IP blocking"""
    ip_block_indicators = [
        "ip blocked", "ip banned", "connection refused",
        "network error", "proxy error", "timeout"
    ]
    return any(indicator in error_msg.lower() for indicator in ip_block_indicators)
